...
(backlog task `default-bundle-cwd`, Dropped): a cwd default silently
treats *any* directory as a bundle — wrong-scope reads and misdirected name-resolved writes.
The successor idea (backlog task `bundle-init-discovery`) is the git
...
(the `research-similar-tools` survey, in this bundle under `research/`).
...
- Registry names open a path to cross-bundle querying (attach each bundle as a DuckDB schema)
  — deferred to the backlog task `cross-bundle-querying`.
